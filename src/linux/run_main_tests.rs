...
use super::*;
#[cfg(test)]
use crate::linux::run_main::install_bwrap_signal_forwarders;
#[cfg(test)]
...
fn split_only_filesystem_policy_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        crate::policy::FileSystemSandboxEntry {
            path: crate::policy::FileSystemPath::Path { path: docs },
            access: crate::policy::FileSystemAccessMode::Read,
        },
    ]);
...
fn root_write_read_only_carveout_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        crate::policy::FileSystemSandboxEntry {
            path: crate::policy::FileSystemPath::Special {
                value: crate::policy::FileSystemSpecialPath::Root,
            },
            access: crate::policy::FileSystemAccessMode::Write,
        },
        crate::policy::FileSystemSandboxEntry {
...
fn cleanup_synthetic_mount_targets_removes_only_empty_mount_targets() {
    ...
    let registrations = register_synthetic_mount_targets(&[
        crate::linux::bwrap_runner::SyntheticMountTarget::missing(&empty_file),
        crate::linux::bwrap_runner::SyntheticMountTarget::missing_empty_directory(&empty_dir),
        crate::linux::bwrap_runner::SyntheticMountTarget::missing(&non_empty_file),
        crate::linux::bwrap_runner::SyntheticMountTarget::missing(&missing_file),
    ]);
...
fn cleanup_synthetic_mount_targets_waits_for_other_active_registrations() {
    ...
    let empty_file = temp_dir.path().join(".git");
    std::fs::write(&empty_file, "").expect("write empty file");
    let target = crate::linux::bwrap_runner::SyntheticMountTarget::missing(&empty_file);
...
fn cleanup_synthetic_mount_targets_removes_transient_file_after_concurrent_owner_exits() {
    let temp_dir = tempfile::TempDir::new().expect("tempdir");
    let empty_file = temp_dir.path().join(".git");
    let first_target = crate::linux::bwrap_runner::SyntheticMountTarget::missing(&empty_file);
...
fn cleanup_protected_create_targets_removes_created_path_and_reports_violation() {
    ...
    let target = crate::linux::bwrap_runner::ProtectedCreateTarget::missing(&dot_git);
