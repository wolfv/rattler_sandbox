//! Bubblewrap launcher (system-bwrap only).
//!
//! Port of `codex-rs/linux-sandbox/src/launcher.rs` with the bundled and Bazel
//! launchers stripped — rattler_sandbox expects bubblewrap to be on PATH
//! (e.g. installed via conda-forge).
...
use crate::linux::exec_util::argv_to_cstrings;
use crate::linux::exec_util::make_files_inheritable;
use crate::linux::find_system_bwrap_in_path;
...
fn system_bwrap_capabilities(system_bwrap_path: &Path) -> Option<SystemBwrapCapabilities> {
    ...
    // reject `--argv0`, so fall back to the no-argv0 path in that case.
