//! Bubblewrap discovery, WSL1 detection, and the `system_bwrap_warning` API.
//!
//! Direct port of `codex-rs/sandboxing/src/bwrap.rs` with codex-specific
//! prose adapted to rattler context.
...
const MISSING_BWRAP_WARNING: &str = concat!(
    "rattler-sandbox could not find bubblewrap on PATH. ",
    "Install `bubblewrap` (e.g. via conda-forge: `pixi global install bubblewrap`).",
);
const USER_NAMESPACE_WARNING: &str =
    "rattler-sandbox uses bubblewrap and needs access to create user namespaces.";
pub(crate) const WSL1_BWRAP_WARNING: &str = concat!(
    "rattler-sandbox uses bubblewrap, which is not supported on WSL1 because ",
    "WSL1 cannot create the required user namespaces. Use WSL2 for sandboxed ",
    "shell commands."
);
