...
use crate::linux::landlock_runner::apply_permission_profile_to_current_thread;
use crate::linux::launcher::exec_bwrap;
...
fn build_bwrap_argv(
...
) -> CodexResult<crate::linux::bwrap_runner::BwrapArgs> {
    ...
    Ok(crate::linux::bwrap_runner::BwrapArgs {
        args: argv,
...
fn run_or_exec_bwrap(bwrap_args: crate::linux::bwrap_runner::BwrapArgs) -> ! {
...
impl ProtectedCreateMonitor {
    fn start(targets: &[crate::linux::bwrap_runner::ProtectedCreateTarget]) -> Option<Self> {
        if targets.is_empty() {
            return None;
        }
...
impl ProtectedCreateWatcher {
    fn new(targets: &[crate::linux::bwrap_runner::ProtectedCreateTarget]) -> Option<Self> {
...
fn register_synthetic_mount_targets(
    targets: &[crate::linux::bwrap_runner::SyntheticMountTarget],
                    ...
                    && synthetic_mount_marker_dir_has_active_synthetic_owner(&marker_dir)
                {
                    match target.kind() {
                        crate::linux::bwrap_runner::SyntheticMountTargetKind::EmptyFile => {
                            crate::linux::bwrap_runner::SyntheticMountTarget::missing(target.path())
                        }
                        crate::linux::bwrap_runner::SyntheticMountTargetKind::EmptyDirectory => {
                            crate::linux::bwrap_runner::SyntheticMountTarget::missing_empty_directory(
                                target.path(),
                            )
                        }
                    }
...
fn register_protected_create_targets(
    targets: &[crate::linux::bwrap_runner::ProtectedCreateTarget],
...
fn remove_synthetic_mount_target(target: &crate::linux::bwrap_runner::SyntheticMountTarget) {
    let path = target.path();
    let metadata = match fs::symlink_metadata(path) {
        Ok(metadata) => metadata,
...
fn run_bwrap_in_child_capture_stderr(bwrap_args: crate::linux::bwrap_runner::BwrapArgs) -> String {
