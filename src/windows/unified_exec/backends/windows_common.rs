...
use crate::windows::elevated::ipc_framed::ResizePayload;
use crate::windows::elevated::ipc_framed::StdinPayload;
...
pub(crate) fn start_runner_pipe_writer(
    ...
    tokio::task::spawn_blocking(move || {
        while let Ok(msg) = outbound_rx.recv() {
            if crate::windows::elevated::ipc_framed::write_frame(&mut pipe_write, &msg).is_err() {
                break;
            }
        }
    });
...
pub(crate) fn start_runner_stdout_reader(
    ...
    std::thread::spawn(move || {
        loop {
            let msg = match crate::windows::elevated::ipc_framed::read_frame(&mut pipe_read) {
                Ok(Some(v)) => v,
                Ok(None) => {
                    send_runner_error(
