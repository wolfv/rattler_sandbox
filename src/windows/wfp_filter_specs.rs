...
// `FILTER_SPECS` is consumed by code paths the published API does not yet
// reach (the legacy WFP installer). Keep the data table around so future
// wiring is one symbol lookup, not a re-port.
#![allow(dead_code)]
