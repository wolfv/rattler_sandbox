...
use crate::windows::helper_materialization::helper_bin_dir;
use crate::windows::logging::log_note;
use crate::windows::path_normalization::canonical_path_key;
use crate::windows::policy::SandboxPolicy;
use crate::windows::setup_error::SetupErrorCode;
use crate::windows::setup_error::SetupFailure;
use crate::windows::setup_error::clear_setup_error_report;
use crate::windows::setup_error::failure;
use crate::windows::setup_error::read_setup_error_report;
use crate::windows::ssh_config_dependencies::ssh_config_dependency_paths;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
use base64::Engine;
...
struct ElevationPayload {
    ...
    otel: Option<crate::windows::otel_stub::StatsigMetricsSettings>,
    real_user: String,
    #[serde(default)]
    refresh_only: bool,
}
...
fn run_setup_exe(
    ...
    let exe_w = crate::windows::winutil::to_wide(&exe);
    let params = quote_arg(&payload_b64);
    let params_w = crate::windows::winutil::to_wide(params);
    let verb_w = crate::windows::winutil::to_wide("runas");
...
pub fn run_elevated_setup(
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: Some(crate::windows::otel_stub::StatsigMetricsSettings::default()),
        refresh_only: false,
    };
