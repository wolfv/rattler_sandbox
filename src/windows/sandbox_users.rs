...
use crate::windows::dpapi_protect;
...
pub fn provision_sandbox_users(
    ...
    crate::windows::setup_main_win::log_line(
        log,
...
pub fn ensure_local_user(name: &str, password: &str, log: &mut File) -> Result<()> {
    ...
    unsafe {
        ...
        if status != NERR_Success {
            ...
            if upd != NERR_Success {
                crate::windows::setup_main_win::log_line(log, &format!("NetUserSetInfo failed for {name} code {upd}"))?;
