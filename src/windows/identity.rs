...
use crate::windows::logging::debug_log;
use crate::windows::policy::SandboxPolicy;
use crate::windows::setup_orchestrator::gather_read_roots;
use crate::windows::setup_orchestrator::gather_write_roots;
use crate::windows::setup_orchestrator::offline_proxy_settings_from_env;
use crate::windows::setup_orchestrator::run_elevated_setup;
use crate::windows::setup_orchestrator::run_setup_refresh_with_overrides;
use crate::windows::setup_orchestrator::sandbox_users_path;
use crate::windows::setup_orchestrator::setup_marker_path;
use crate::windows::setup_orchestrator::SandboxNetworkIdentity;
use crate::windows::setup_orchestrator::SandboxUserRecord;
use crate::windows::setup_orchestrator::SandboxUsersFile;
use crate::windows::setup_orchestrator::SetupMarker;
use anyhow::anyhow;
use anyhow::Context;
use anyhow::Result;
...
pub fn require_logon_sandbox_creds(
    ...
    let sandbox_dir = crate::windows::setup_orchestrator::sandbox_dir(codex_home);
    ...
    if identity.is_none() {
        if let Some(reason) = &setup_reason {
            crate::windows::logging::log_note(
                &format!("sandbox setup required: {reason}"),
                Some(&sandbox_dir),
            );
        } else {
            crate::windows::logging::log_note("sandbox setup required", Some(&sandbox_dir));
        }
        run_elevated_setup(
            crate::windows::setup_orchestrator::SandboxSetupRequest {
                policy,
                policy_cwd,
                command_cwd,
                env_map,
            ...
            crate::windows::setup_orchestrator::SetupRootOverrides {
