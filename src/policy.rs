...
//!
//! Combined port of:
//!   - codex-rs/protocol/src/permissions.rs       (FileSystem* / NetworkSandboxPolicy / ReadDenyMatcher)
//!   - codex-rs/protocol/src/protocol.rs (slice)  (SandboxPolicy / WritableRoot / NetworkAccess)
//!   - codex-rs/protocol/src/models.rs (slice)    (PermissionProfile / AdditionalPermissionProfile)
//!   - codex-rs/protocol/src/config_types.rs (slice) (WindowsSandboxLevel)
//!
//! Trimmed: `schemars::JsonSchema`, `ts_rs::TS`, `strum_macros::Display` derives
//! and their attribute lines are dropped.
...
use crate::path::canonicalize_preserving_symlinks;

// ---------------------------------------------------------------------------
// Top-level sandbox policy + writable-root types
// (originally in codex-rs/protocol/src/protocol.rs)
// ---------------------------------------------------------------------------

/// Whether outbound network access is available to the sandboxed process.
...
/// Top-level sandbox policy (legacy enum). [`PermissionProfile`] is the richer
/// successor; both are accepted by the manager.
...
// ---------------------------------------------------------------------------
// Permission profile (richer, used as the primary policy interface)
// (originally in codex-rs/protocol/src/models.rs)
// ---------------------------------------------------------------------------

/// Bundles a [`FileSystemSandboxPolicy`] and a [`NetworkSandboxPolicy`] into a
/// single named profile. Per-call overrides are expressed via
...
impl PermissionProfile {
    ...
    pub fn read_only() -> Self {
        Self {
            file_system: FileSystemSandboxPolicy::default(),
            network: NetworkSandboxPolicy::Restricted,
        }
    }

    pub fn workspace_write(_cwd: &Path) -> Self {
        Self {
            file_system: FileSystemSandboxPolicy::workspace_write(&[], false, false),
            network: NetworkSandboxPolicy::Restricted,
        }
    }

    /// Resolve to the runtime policies the engines consume.
    pub fn to_runtime_permissions(&self) -> (FileSystemSandboxPolicy, NetworkSandboxPolicy) {
        (self.file_system.clone(), self.network)
    }

    /// Returns a clone of the filesystem half of this profile.
    pub fn file_system_sandbox_policy(&self) -> FileSystemSandboxPolicy {
        self.file_system.clone()
    }

    /// Returns the network half of this profile.
    pub fn network_sandbox_policy(&self) -> NetworkSandboxPolicy {
        self.network
    }

    /// Build a [`PermissionProfile`] from runtime policies.
    ...
    pub fn to_legacy_sandbox_policy(&self, cwd: &Path) -> Result<SandboxPolicy, String> {
        crate::policy_transforms::permission_profile_to_legacy_sandbox_policy(self, cwd)
    }
}

/// Per-call adjustment to a [`PermissionProfile`].
...
// ---------------------------------------------------------------------------
// Windows sandbox configuration (originally in config_types.rs)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Filesystem / network policy types (originally in permissions.rs)
// ---------------------------------------------------------------------------
