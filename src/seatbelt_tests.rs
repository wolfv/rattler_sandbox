...
fn assert_seatbelt_denied(stderr: &[u8], path: &Path) {
    ...
    // Newer bash versions prepend `line N: ` before the path. Both forms count
    // as a sandbox-denied write to `path`.
    let denied_path = stderr.contains(&format!("bash: {path_str}: Operation not permitted"))
        || stderr.contains(&format!(": {path_str}: Operation not permitted"));
    let denied_apply = stderr.contains("sandbox-exec: sandbox_apply: Operation not permitted");
    assert!(denied_path || denied_apply, "unexpected stderr: {stderr}");
}
...
// `TestConfigReloader` and the test that uses it require the full
// `codex_network_proxy` runtime which we strip. Re-enable when a richer
// `NetworkProxy` lands.
